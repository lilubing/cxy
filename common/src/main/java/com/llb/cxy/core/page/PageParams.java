...
import com.llb.cxy.core.constants.CommonConstants;
...
public class PageParams extends Page implements Serializable {
    private static final long serialVersionUID = -1710273703152963319L;
    ...
    public String getOrderBy() {
        if (MyStringUtils.isBlank(order)) {
            order = "asc";
        }
        if (MyStringUtils.isNotBlank(sort)) {
            this.setOrderBy(String.format("%s %s", MyStringUtils.camelToUnderline(sort), order));
        }
        return orderBy;
    }
